...
export function Timeline() {
    ...
    return (<section className="px-6 sm:px-12 lg:px-20 py-16 max-w-7xl mx-auto">
      <h2 className="text-3xl sm:text-4xl text-center">Build Timeline</h2>

      <div className="mt-8 flex flex-wrap justify-center gap-3">
        {projects.map((p) => {
            const isActive = p.id === active.id;
            return (<button key={p.id} onClick={() => setActiveId(p.id)} className={`rounded-full border px-4 py-1.5 text-sm transition-colors ${isActive
      ...
      <div className="mt-10 rounded-xl border border-border bg-card p-6 sm:p-8">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {active.highlights.map((h) => (<span key={h} className="rounded-full border border-primary/40 bg-primary/10 text-primary px-3 py-1 text-xs">
              {h}
            </span>))}
        </div>
        ...
        <ul className="mt-3 space-y-2 text-sm text-muted-foreground">
          {active.next.map((n) => (<li key={n} className="flex gap-2"><span className="text-primary">•</span>{n}</li>))}
        </ul>
        <div className="mt-4 flex flex-wrap gap-2">
          {active.tools.map((t) => (<span key={t} className="text-[11px] text-muted-foreground border border-border rounded px-2 py-0.5">
              {t}
            </span>))}
        </div>
      </div>
      ...
      <div className="relative mt-8 pl-6">
        ...
        <div className="space-y-4">
          {active.events.map((e) => (<div key={e.title} className="relative">
