...
export function Hobbies() {
    return (<section className="px-6 sm:px-12 lg:px-20 py-16 max-w-7xl mx-auto">
      ...
      <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {HOBBIES.map((h) => (<article key={h.title} className="rounded-xl border border-border bg-card overflow-hidden hover:border-primary/60 transition-colors">
            <div className="h-40 bg-background overflow-hidden">
              <img src={h.img} alt={h.title} className="w-full h-full object-cover grayscale"/>
            </div>
