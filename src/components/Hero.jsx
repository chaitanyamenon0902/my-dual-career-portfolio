...
function useTypewriter(words, { typeMs = 75, deleteMs = 40, holdMs = 1400 } = {}) {
...
export function Hero() {
    ...
    return (<section className="px-6 sm:px-12 lg:px-20 pt-28 pb-16 max-w-7xl mx-auto">
      <h1 className="text-4xl sm:text-6xl lg:text-7xl leading-tight tracking-tight">
        ...
        <span className="inline-block w-[0.55ch] ml-1 align-middle bg-primary animate-pulse" style={{ height: "0.85em" }} aria-hidden/>
      </h1>

      <div className="mt-20 flex flex-col gap-6">
        <div className="relative w-32 h-32">
          <img src={portrait} alt="Chaitanya Menon" className="w-32 h-32 rounded-full object-cover ring-glow-primary"/>
        ...
        <div className="flex flex-wrap gap-3 mt-2">
          {skillsByMode[mode].map((s) => (<div key={s.name} className="group relative">
              <div className="w-11 h-11 rounded-full bg-surface-elevated border border-border flex items-center justify-center hover:border-primary hover:-translate-y-0.5 transition-all" aria-label={s.name}>
                <img src={`https://cdn.simpleicons.org/${s.slug}/${s.color}`} alt={s.name} className="w-5 h-5" loading="lazy"/>
              </div>
